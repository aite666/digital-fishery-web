  ...
  <el-card class="form-container" shadow="never">
    ...
    >
      ...
      <el-form-item label="区块名称：" prop="blockName">
        <block-select
          @block="getBlock"
          :blockSelectedId="adviceDetail.blockId"
        ></block-select>
      </el-form-item>
      ...
      <el-form-item>
        <el-button type="primary" @click="onSubmit('adviceForm')"
          >提交</el-button
        >
        <el-button v-if="!isEdit" @click="resetForm('adviceForm')"
          >重置</el-button
        >
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$confirm("是否提交数据", "提示", {
            ...
            if (this.isEdit) {
              updateAdvice(this.$route.query.id, this.adviceDetail).then(
                (response) => {
                  this.$message({
                    message: "修改成功",
                    type: "success",
