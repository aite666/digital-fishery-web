<template>
  ...
  <div class="app-container">
    ...
    <el-card class="operate-container" shadow="never">
      ...
      <span>数据列表</span>
      <el-button class="btn-add" @click="handleAddInspection()" size="mini">
        添加
      </el-button>
    </el-card>
    <div class="table-container">
      ...
      >
        ...
        <el-table-column label="环境数据" align="center">
          <template slot-scope="scope">{{
            scope.row.environmentData
          }}</template>
        </el-table-column>
        <el-table-column label="品种性状描述" align="center">
          <template slot-scope="scope">{{
            scope.row.characterDescription
          }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="160" align="center">
          <template slot-scope="scope">
            <p>
              <el-button
                size="mini"
                @click="handleUpdateInspection(scope.$index, scope.row)"
                >编辑
              </el-button>
              <el-button
                size="mini"
                type="danger"
                @click="handleDeleteInspection(scope.$index, scope.row)"
                >删除
              </el-button>
            </p>
          </template>
