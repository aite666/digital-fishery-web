<template>
  <div class="app-container">
    ...
    <el-card class="operate-container" shadow="never">
      ...
      <span>数据列表</span>
      <el-button class="btn-add" @click="handleAddEnterprise()" size="mini">
        添加
      </el-button>
    </el-card>
    <div class="table-container">
      <el-table
        ref="infoEnterpriseTable"
        :data="list"
        style="width: 100%"
      ...
      >
        ...
        <el-table-column label="成立时间" width="120" align="center">
          <template slot-scope="scope">{{
            scope.row.creationTime | formatCreationTime
          }}</template>
        </el-table-column>
        <el-table-column label="经营内容" align="center">
          <template slot-scope="scope">{{
            scope.row.businessContent
          }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="160" align="center">
          <template slot-scope="scope">
            <p>
              <el-button
                size="mini"
                @click="handleUpdateEnterprise(scope.$index, scope.row)"
                >编辑
              </el-button>
              <el-button
                size="mini"
                type="danger"
                @click="handleDeleteEnterprise(scope.$index, scope.row)"
                >删除
              </el-button>
            </p>
          </template>
      ...
      fetchList(this.listQuery).then((response) => {
        ...
        if (this.enterpriseId && this.enterpriseId != -1) {
          ...
          for (let i = 0; i < list.length; i++) {
            if (list[i].id == this.enterpriseId) {
              newList.push(list[i]);
            }
          }
          this.list = newList;
