  ...
  methods: {
    getProductCateList() {
      fetchListWithChildren().then((response) => {
        ...
        for (let i = 0; i < list.length; i++) {
          ...
          if (list[i].children != null && list[i].children.length > 0) {
            for (let j = 0; j < list[i].children.length; j++) {
              ...
              productCateSelectedInfo[list[i].children[j].id.toString()] =
                list[i].children[j];
              parentIdInfo[list[i].children[j].id.toString()] = [
                list[i].id,
                list[i].children[j].id,
              ];
            }
          }
          productCateOptions.push({
            label: list[i].name,
            value: list[i].id,
            children: children,
          });
        ...
        if (
          this.selectedId &&
          this.selectedId.toString() in this.parentIdInfo
        ) {
          this.selectProductCateValue = this.parentIdInfo[
    ...
    sendProductCate() {
      //1.子组件通过子定义事件的方式将productCate信息传给父组件
      if (this.selectProductCateValue.length > 0) {
        this.productCateSelectedId = this.selectProductCateValue[
          this.selectProductCateValue.length - 1
        ];
        this.productCateSelected = this.productCateSelectedInfo[
          this.productCateSelectedId.toString()
        ];
      } else {
