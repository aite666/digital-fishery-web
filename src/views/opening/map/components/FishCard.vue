  ...
  methods: {
    getChartData(blockId) {
      ...
      getChartData(params).then((response) => {
        ...
        let dateList = [];
        let batchQuantityList1 = [];
        let batchQuantityList2 = [];
        let batchNumList = [];
        let max1 = 0;
        let max2 = 0;
        for (let i = 0; i < list.length; i++) {
          ...
          batchQuantityList1.push(item.batchQuantity1);
          if (item.batchQuantity1 > max1) {
            max1 = item.batchQuantity1;
          }
          batchQuantityList2.push(item.batchQuantity2);
          if (item.batchQuantity2 > max1) {
            max1 = item.batchQuantity2;
          }
          batchNumList.push(item.batchNum);
          if (item.batchNum > max2) {
            max2 = item.batchNum;
          }
        }
        ...
        this.defaultOption.series[0].data = batchQuantityList1;
        this.defaultOption.series[1].data = batchQuantityList2;
        this.defaultOption.series[2].data = batchNumList;
        setTimeout(() => {
